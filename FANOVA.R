...
FANOVA<-function(data_pop1,data_pop2,year=1959:2020,age= 0:100,n_prefectures=51,n_populations=2)
{
  ...
  FGE_means<-mu_hat(data_pop1,data_pop2,n_year
                    ,n_prefectures,n_age,n_populations,row_par)
  
  #functional row effect
  FRE_mean<-FRE_means(data_pop1,data_pop2,n_year
                      ,n_prefectures,n_age,n_populations,row_par)
  
  #Functional column effect
  FCE_mean<-FCE_means(data_pop1,data_pop2,n_year
                      ,n_prefectures,n_age,n_populations,row_par)
  
  return(list(FGE_mean=FGE_means,FRE_mean=t(FRE_mean),FCE_mean=t(FCE_mean)))
}

#' Return from the function FANOVA
#' 
#' @return Return a list containing grand effect,row effect and column effect computed based on means.
#' @return FGE_mean, a vector of dimension p
#' @return FRE_mean, a matrix of dimension length(row_partition_index) by p.
#' @return FCE_mean, a matrix of dimension length(column_partition_index) by p
