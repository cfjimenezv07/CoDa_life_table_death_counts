...
################################################################################
all_male<-t(list.cbind(All_France_male_qx))
all_female<-t(list.cbind(All_France_female_qx))
...
plot.fds(curves_male,main="France: male data (1968-2021)")
colnames(Female)<-1:dim(Female)[2]
curves_female<-rainbow::fts(x=1:n_age, y=Female , xname='Age',yname='Life-table death counts')
plot.fds(curves_female,main="France: female data (1968-2021)")
par(mfrow=c(1,1))

################################################
#Plots for Figure 1c, 1d.
################################################

# Plot for Paris 
savepdf("Paris_female")
Female<- All_France_female_qx[[71]]
colnames(Female)<-1:dim(Female)[2]
curves_female<-rainbow::fts(x=1:n_age, y=Female , xname='Age',yname='Life-table death counts')
plot.fds(curves_female,ylim=c(0,5200),xlim=c(0,105))
dev.off()
...
savepdf("Paris_male")
Male<-All_France_male_qx[[71]]
colnames(Male)<-1:dim(Male)[2]
curves_male<-rainbow::fts(x=1:n_age, y=Male , xname='Age',yname='')
plot.fds(curves_male,ylim=c(0,5200),xlim=c(0,105))
dev.off()
...
for (i in 1: n_prefectures) {
  Gini_female[,i]=ts(apply(All_France_female_qx[[i]], 2, gini), start = 1968, end = 2021)
  Gini_male[,i]=ts(apply(All_France_male_qx[[i]], 2, gini), start = 1968, end = 2021)
}
...
alpha_female         <- list()
for (i in 1:n_prefectures) {
  transformation <- log_ratio_trans(dat_1=t(All_France_male_qx[[i]])+1e-5,dat_2=t(All_France_female_qx[[i]])+1e-5)
...
for (i in 1:n_prefectures) {
  ...
  dat_1      <- t(All_France_male_qx[[i]])+1e-5
  dat_2      <- t(All_France_female_qx[[i]])+1e-5
...
rownames(Errors_mean_rolling)<-name_departments
rownames(Errors_sd_rolling)<-name_departments
...
colnames(Emp_cov_80) = c("Pointwise 80% coverage",
                         ...
                         "80% interval score",
                         "CPD Pointwise 80% coverage")
rownames(Emp_cov_80)<-name_departments
...
colnames(Emp_cov_95) = c("Pointwise 95% coverage",
                         ...
                         "95% interval score",
                         "CPD Pointwise 95% coverage")
rownames(Emp_cov_95)<-name_departments
...
# Based on rolling window approach
...
rownames(Errors_mean_basedmeans_rolling)<-name_departments
rownames(Errors_sd_basedmeans_rolling)<-name_departments
...
colnames(Emp_cov_80_means) = c("Pointwise 80% coverage",
                               ...
                               "80% interval score",
                               "CPD Pointwise 80% coverage")
rownames(Emp_cov_80_means)<-name_departments
...
colnames(Emp_cov_95_means) = c("Pointwise 95% coverage",
                               ...
                               "95% interval score",
                               "CPD Pointwise 95% coverage")
rownames(Emp_cov_95_means)<-name_departments
