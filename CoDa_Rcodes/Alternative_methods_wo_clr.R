...
# Alternative methods of GSY19 and TNH23 for CoDA without the CoDA transformation
...
USA_male   <- All_USA_male_qx 
USA_female <- All_USA_female_qx
...
# saveRDS(predictions_GAO_female,paste0(dirl.p,"predictions_GAO_female_wo_clr.rds"))
# saveRDS(predictions_TNH_female,paste0(dirl.p,"predictions_TNH_female_wo_clr.rds"))
...
for (i in 1:n_states) {
  ...
  for (j in 1:h_max) {
    ...
    tnh_aux <- as.matrix(predictions_TNH_female_wo_clr[[j]][i,,]); if (j < h_max) tnh_aux <- t(tnh_aux)
    gao_aux <- as.matrix(predictions_GAO_female_wo_clr[[j]][i,,]); if (j < h_max) gao_aux <- t(gao_aux)
    forecast_female_TNH[[i]][,j,] <- cbind(tnh_aux, aux)
    forecast_female_GSY[[i]][,,j] <- cbind(gao_aux, aux)
  }
}

# saveRDS(forecast_female_TNH,paste0(dirl.p,"forecast_female_TNH_w0_clr.rds"))
# saveRDS(forecast_female_GSY,paste0(dirl.p,"forecast_female_GSY_w0_clr.rds"))
...
for (i in 1:n_states) {
  ...
  Forc_female_TNH_1   <- forecast_female_TNH[[i]]
  Forc_female_GSY_1   <- forecast_female_GSY[[i]]
  
  
  for (j in 1:h_max) {
    data_TNH <- Forc_female_TNH_1[,j,]
    data_GSY <- Forc_female_GSY_1[,j,]
    


    Forc_female_TNH <-Remove_zeros_norm(data_TNH)
    Forc_female_GSY <-Remove_zeros_norm(data_GSY)

    errors_female_TNH  <- rowMeans(sapply( 1:(h_max-j+1),pairwise_errors, Forc_female_TNH,dat_female[,(n_year-h_max+j):(n_year)])) 
    errors_female_GSY  <- rowMeans(sapply( 1:(h_max-j+1),pairwise_errors, Forc_female_GSY,dat_female[,(n_year-h_max+j):(n_year)])) 
...
All_errors_female_w0_clr<-list( TNH_error_KLD_female,GAO_error_KLD_female,
                         TNH_error_JS1_female,GAO_error_JS1_female,
                         TNH_error_JS2_female,GAO_error_JS2_female)



# saveRDS(All_errors_female,paste0(dirl.p,"All_errors_female_TNH_GSY_w0_clr.rds"))
...
# saveRDS(predictions_GAO_male,paste0(dirl.p,"predictions_GAO_male_wo_clr.rds"))
# saveRDS(predictions_TNH_male,paste0(dirl.p,"predictions_TNH_male_wo_clr.rds"))
...
for (i in 1:n_states) {
  ...
  for (j in 1:h_max) {
    aux <- matrix(0, ncol = j-1, nrow = n_age)
    tnh_aux <- as.matrix(predictions_TNH_male_wo_clr[[j]][i,,]); if (j < h_max) tnh_aux <- t(tnh_aux)
    gao_aux <- as.matrix(predictions_GAO_male_wo_clr[[j]][i,,]); if (j < h_max) gao_aux <- t(gao_aux)
    forecast_male_TNH[[i]][,j,] <- cbind(tnh_aux, aux)
    forecast_male_GSY[[i]][,,j] <- cbind(gao_aux, aux)
  }
}

# saveRDS(forecast_male_TNH,paste0(dirl.p,"forecast_male_TNH_w0_clr.rds"))
# saveRDS(forecast_male_GSY,paste0(dirl.p,"forecast_male_GSY_w0_clr.rds"))
...
for (i in 1:n_states) {
  ...
  Forc_male_TNH_1   <- forecast_male_TNH[[i]]
  Forc_male_GSY_1   <- forecast_male_GSY[[i]]
  
  
  for (j in 1:h_max) {
    
    data_TNH <- Forc_male_TNH_1[,j,]
    data_GSY <- Forc_male_GSY_1[,j,]
    Forc_male_TNH <-Remove_zeros_norm(data_TNH)
    Forc_male_GSY <-Remove_zeros_norm(data_GSY)
    
    errors_male_TNH  <- rowMeans(sapply( 1:(h_max-j+1),pairwise_errors, Forc_male_TNH,dat_male[,(n_year-h_max+j):(n_year)])) 
    errors_male_GSY  <- rowMeans(sapply( 1:(h_max-j+1),pairwise_errors, Forc_male_GSY,dat_male[,(n_year-h_max+j):(n_year)])) 
...
All_errors_male_w0_clr<-list( TNH_error_KLD_male,GAO_error_KLD_male,
                         TNH_error_JS1_male,GAO_error_JS1_male,
                         TNH_error_JS2_male,GAO_error_JS2_male)



# saveRDS(All_errors_male,paste0(dirl.p,"All_errors_male_TNH_GSY_w0_clr.rds"))


# v3 <- cbind(apply(All_errors_female_w0_clr[[1]],1,mean),apply(All_errors_female_w0_clr[[5]],1,mean),apply(All_errors_male_w0_clr[[1]],1,mean),
#             apply(All_errors_male_w0_clr[[5]],1,mean),
#             +             apply(All_errors_female_w0_clr[[2]],1,mean),apply(All_errors_female_w0_clr[[6]],1,mean),
#             apply(All_errors_male_w0_clr[[2]],1,mean),apply(All_errors_male_w0_clr[[6]],1,mean))*100
# 
